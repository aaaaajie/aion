...
@pytest.mark.asyncio
async def test_evidence_is_private_immutable_and_survives_source_cleanup(
    tmp_path: Path,
) -> None:
    ...
    await service.register_agent(
        "run", agent_id="chief", role="chief", initial_prompt="x"
    )
    await service.register_agent(
        "run",
        agent_id="challenge",
    ...
    context = CapabilityContext(
        run_id="run", agent_id="execution", role="worker", unique_code="c"
    )
