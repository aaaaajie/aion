...
description: 'HTTP Parameter Pollution (HPP): duplicate query/body keys parsed differently
  by servers, proxies, WAFs, and app frameworks. Use when filters and application
  layers disagree on which value wins, enabling bypass, SSRF second URL, logic abuse,
  or CSRF token confusion.'
---

# Http Parameter Pollution Skill
...
- the task mentions `pollution`
- the task mentions `hpp`
