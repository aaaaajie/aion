...
description: Web cache deception and poisoning playbook. Use when CDN, reverse proxy,
  or application caching may serve sensitive authenticated content to other users
  due to path confusion or cache key manipulation.
---
...
- the task mentions `cache`
- the task mentions `deception`
- the task mentions `poisoning`
...
Also load [references/CACHE_POISONING_TECHNIQUES.md](references/CACHE_POISONING_TECHNIQUES.md) when you need:
