...
description: >-
  PHP file-upload audit for a confirmed upload surface or PHP upload implementation,
  including multipart handling, filename/MIME validation, storage paths, executable
  uploads, and web shells. Not for login, default credentials, generic 403 bypass,
  ordinary file reads, or broad PHP reconnaissance.
when_to_use: >-
  Use only when the assignment identifies an upload endpoint, multipart upload flow,
  uploaded-file handling code, upload directory, or a hypothesis that requires writing
  and executing an uploaded PHP file.
---
