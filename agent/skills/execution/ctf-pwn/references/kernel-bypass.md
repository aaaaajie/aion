...
See `kernel.md - modprobe_path Overwrite` (resource: `references/kernel.md#modprobe_path-overwrite`) for the full technique, trigger sequence, and ROP payload.
...
See `kernel.md - core_pattern Overwrite` (resource: `references/kernel.md#core_pattern-overwrite`) for the full technique and how to find the `core_pattern` address.
...
- **Bypass:** Use kernel ROP (kROP) chains — all gadgets from kernel `.text`. See `kernel.md - Kernel ROP` (resource: `references/kernel.md#kernel-rop-with-prepare_kernel_cred--commit_creds`).
...
|-----------|--------|--------|
| SMEP | Executing userland pages from kernel | kROP (kernel ROP chain) — see `kernel.md` (resource: `references/kernel.md#kernel-rop-with-prepare_kernel_cred--commit_creds`) |
| SMAP | Accessing userland memory from kernel | kROP with heap-resident chain, `stac`/`clac` gadgets |
| No SMEP/SMAP | (nothing) | `ret2usr` (resource: `references/kernel.md#ret2usr-no-smepsmap`) — directly call userland privesc function |
