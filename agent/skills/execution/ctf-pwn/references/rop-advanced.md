...
For core ROP chain building, ret2csu, bad character bypass, exotic gadgets, and stack pivot via xchg, see `rop-and-shellcode.md` (resource: `references/rop-and-shellcode.md`).
...
See `advanced.md` (resource: `references/advanced.md`) for: conditional buffer address restrictions, shellcode construction without relocations (call/pop trick), seccomp analysis from disassembly, `scmp_arg_cmp` struct layout.
