...
For tty_struct kROP (kernel Return-Oriented Programming), userfaultfd race stabilization, SLUB internals, cross-cache attacks, and DiceCTF 2026 kernel patterns, see `kernel-techniques.md` (resource: `references/kernel-techniques.md`).

For protection bypass techniques (KASLR, FGKASLR, KPTI, SMEP, SMAP), GDB debugging, initramfs workflow, and exploit templates, see `kernel-bypass.md` (resource: `references/kernel-bypass.md`).
...
See also: `kernel-techniques.md` (resource: `references/kernel-techniques.md`) for additional kernel exploitation techniques.
