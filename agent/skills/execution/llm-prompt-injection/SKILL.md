...
description: LLM prompt injection playbook. Use when testing AI/LLM applications for direct injection, indirect injection via RAG/browsing, tool abuse, data exfiltration, MCP security risks, and defense bypass techniques.
---

# Llm Prompt Injection Skill
...
- `xss-cross-site-scripting` (related Skill; use `skill_list` to locate it) for parallels between XSS (injecting into HTML context) and prompt injection (injecting into LLM context)
- ssrf-server-side-request-forgery when prompt injection chains into SSRF via tool calls

### Advanced Reference

Also load [references/JAILBREAK_PATTERNS.md](references/JAILBREAK_PATTERNS.md) when you need:
...
Effectiveness varies by model and version. See [references/JAILBREAK_PATTERNS.md](references/JAILBREAK_PATTERNS.md) for comprehensive catalog.
