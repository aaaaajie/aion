...
- `jndi-injection` (related Skill; use `skill_list` to locate it) when deserialization leads to JNDI lookup (e.g., post-JDK 8u191 bypass via LDAP → deserialization)
...
- `ghost-bits-cast-attack` (related Skill; use `skill_list` to locate it) when a WAF blocks your BCEL ClassLoader or Fastjson `@type` payload — Ghost Bits wraps each bytecode byte in a Unicode char whose low 8 bits match, yielding a payload the WAF cannot fingerprint

### Advanced Reference

Also load `references/JAVA_GADGET_CHAINS.md` (resource: `references/JAVA_GADGET_CHAINS.md`) when you need:
