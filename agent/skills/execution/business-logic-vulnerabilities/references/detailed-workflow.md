...
> **AI LOAD INSTRUCTION**: Business logic flaws are scanner-invisible and high-reward on bug bounty. This skill covers race conditions, price manipulation, workflow bypass, coupon/referral abuse, negative values, and state machine attacks. These require human reasoning, not automation. For specific exploitation techniques (payment precision/overflow, captcha bypass, password reset flaws, user enumeration), load the companion `references/SCENARIOS.md` (resource: `references/SCENARIOS.md`). For the workflow approach itself (modeling → state machine → attack-surface matrix → human judgement) load `references/METHODOLOGY.md` (resource: `references/METHODOLOGY.md`). For the per-module check items load `references/CHECKLIST.md` (resource: `references/CHECKLIST.md`).

### Companion files

| File | When to load |
|---|---|
| `references/METHODOLOGY.md` (resource: `references/METHODOLOGY.md`) | Need the 5-phase workflow, attack-surface 5×N matrix, human-judgement decision tree |
| `references/CHECKLIST.md` (resource: `references/CHECKLIST.md`) | Going through a target module-by-module (login / register / payment / IDOR / privacy) and want every line item with why+verify |
| `references/SCENARIOS.md` (resource: `references/SCENARIOS.md`) | Drilling deeper into payment precision/overflow, captcha bypass, password reset, enumeration, frontend bypass |

### Extended Scenarios

Also load `references/SCENARIOS.md` (resource: `references/SCENARIOS.md`) when you need:
...
- `upload insecure files` (related Skill; use `skill_list` to locate it)
...
For the formal 5-phase workflow — Business Modeling → State Machine → Attack-Surface Matrix → Checklist-Driven Testing → Human Judgement — load **`references/METHODOLOGY.md` (resource: `references/METHODOLOGY.md`)**. It includes a single-page decision tree (`Q1 ~ Q7`) for "I'm staring at a request and don't know what to try first".

---

## 10. HIGH-IMPACT CHECKLISTS

For the full per-module list (login / register / password recovery / payment / coupon / order / IDOR / privacy / VIP / URL redirect / cookie & token / race / comments) with `why` and `verify` columns — load **`references/CHECKLIST.md` (resource: `references/CHECKLIST.md`)**.
...
tooling per item, load **`references/CHECKLIST.md` (resource: `references/CHECKLIST.md`)**. This section
...
4. Cross-reference with `references/METHODOLOGY.md` Q1~Q7 decision tree when an item
...
5. For deep payloads / curl-ready commands / Burp screenshots per item,
   load `references/CHECKLIST.md` (full triplets) and `references/SCENARIOS.md` (real cases).
