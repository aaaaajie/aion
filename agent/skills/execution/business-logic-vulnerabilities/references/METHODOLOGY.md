...
> Companion to `SKILL.md` (resource: `SKILL.md`) and `CHECKLIST.md` (resource: `references/CHECKLIST.md`). Distilled from real-world payment / captcha / authentication / data-exposure logic flaw cases and live Java code-audit walkthroughs.
...
实际操作阶段直接打开 `CHECKLIST.md` (resource: `references/CHECKLIST.md`)，**按业务模块从上到下走**，每条都问：
...
- `SKILL.md` (resource: `SKILL.md`) — 业务逻辑漏洞 attack playbook（英文）
- `SCENARIOS.md` (resource: `references/SCENARIOS.md`) — 支付精度/竞态/验证码/找回密码/枚举的细化 scenario（英文）
- `CHECKLIST.md` (resource: `references/CHECKLIST.md`) — 按业务模块分组的可勾选检查项（双语）
- 蒸馏原始素材（外部，未入库）：
  - 业务逻辑漏洞审计课程录像
