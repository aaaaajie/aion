...
> Companion to `SKILL.md` (resource: `SKILL.md`), `SCENARIOS.md` (resource: `references/SCENARIOS.md`), `METHODOLOGY.md` (resource: `references/METHODOLOGY.md`). Distilled from instructor-led classes and the original `逻辑漏洞checklist.pdf` curriculum, every entry below has a **why** (root cause) and a **verify** (how to reproduce).
...
2. **第二轮**：把第一轮挂红的（疑似漏洞）拿到 `METHODOLOGY.md §6.1 三个追问` (resource: `references/METHODOLOGY.md`) 复盘，每条跑业务侧验证（看 VIP 真增加了吗？钱真扣别人的吗？数据真返了不该返的吗？）。
...
- `METHODOLOGY.md` (resource: `references/METHODOLOGY.md`) — 五阶段方法论 / 单页决策树
- `SKILL.md` (resource: `SKILL.md`) — 业务逻辑漏洞 attack playbook
- `SCENARIOS.md` (resource: `references/SCENARIOS.md`) — 支付/竞态/找回密码/枚举/上传细化场景
