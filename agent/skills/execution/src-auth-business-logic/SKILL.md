...
3. Trace server-side checks for ownership, role, tenant, state, amount, count, and
   replay protection. Treat client-side checks as untrusted hints.
4. Choose one differential hypothesis: role substitution, object substitution,
   state skip, parameter omission/addition, replay, or concurrent transition.
5. Capture a normal control interaction first. Change one variable at a time and
   preserve the same authentication context where the hypothesis requires it.
6. Use the existing HTTP session/request/output tools to compare status, body,
   side effects, and subsequent reads. Do not repeat identical requests.
7. Verify the impact on the intended object or workflow state, then deduplicate
...
`system_http_response` for controlled verification. Use `execution_report` once
