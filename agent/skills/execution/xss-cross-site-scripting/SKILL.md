...
- the task mentions `xss`
- the task mentions `cross`
- the task mentions `site`
- the task mentions `scripting`
...
Also load [references/ADVANCED_XSS_TRICKS.md](references/ADVANCED_XSS_TRICKS.md) when you need:
...
- `upload insecure files` (related Skill; use `skill_list` to locate it) when you need the full upload path: validation, storage, preview, and sharing behavior
