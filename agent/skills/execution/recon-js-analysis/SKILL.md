...
description: >-
  Front-end JavaScript reconnaissance for fetched HTML or JS assets: bundles, source
  maps, webpack chunks, client API routes, embedded keys, signing logic, and hidden
  browser-side interfaces. Not for raw port scanning, generic service fingerprinting,
  or tasks with no web front end.
when_to_use: >-
  Use when the assignment requires inspecting HTML or JavaScript assets, reconstructing
  client-side routes or signing logic, finding source maps or chunks, or extracting
  browser-visible endpoints and secrets.
---

# Recon Js Analysis Skill
...
- the task mentions `webpack`
- the task mentions `source`
