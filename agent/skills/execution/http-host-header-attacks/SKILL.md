...
description: HTTP Host header injection and routing abuse playbook. Use when the application
  trusts the Host header for generating URLs, routing requests, or access control
  — enabling password reset poisoning, web cache poisoning, SSRF via routing, and
  virtual host bypass.
when_to_use: >-
  Use when evidence or the assignment specifically concerns Host or X-Forwarded-Host
  trust, virtual-host routing, absolute URL generation, password-reset links, cache
  keys, or host-based access controls. Not for generic HTTP enumeration, port scanning,
  or unrelated header collection.
---

# Http Host Header Attacks Skill
...
- the task mentions `http`
- the task mentions `host`
- the task mentions `header`
...
- `web-cache-deception` (related Skill; use skill_list to locate it) when Host injection is combined with cache behavior
...
- `request-smuggling` (related Skill; use skill_list to locate it) when smuggling enables Host header manipulation past front-end validation
