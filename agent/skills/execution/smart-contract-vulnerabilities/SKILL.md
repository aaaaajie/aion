...
description: Smart contract vulnerability playbook and smart-contract-vulnerabilities router for Solidity EVM reentrancy delegatecall signature replay. Use when auditing Solidity/EVM contracts for reentrancy, integer overflow, access control, delegatecall, flash loan, signature replay, and MEV-related attack patterns.
---

# Smart Contract Vulnerabilities Skill
...
- `defi-attack-patterns` (related Skill; use `skill_list` to locate it) when the vulnerability is part of a DeFi protocol exploit (flash loans, oracle manipulation, governance attacks)
- `deserialization-insecure` (related Skill; use `skill_list` to locate it) when the target is off-chain infrastructure deserializing blockchain data

### Advanced Reference

Also load [references/SOLIDITY_VULN_PATTERNS.md](references/SOLIDITY_VULN_PATTERNS.md) when you need:
...
│   ├── Yes → Load `defi-attack-patterns` (related Skill; use `skill_list` to locate it)
│   │   ├── Flash loan vectors
│   │   ├── Oracle manipulation
│   │   └── MEV exposure
│   └── No → Continue
