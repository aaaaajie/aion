---
name: jndi-injection
description: JNDI injection playbook. Use when Java applications perform JNDI lookups
  with attacker-controlled names, especially via Log4j2, Spring, or any code path
  reaching InitialContext.lookup().
---

# Jndi Injection Skill
...
- `deserialization-insecure` (related Skill; use skill_list to locate it) when JNDI leads to deserialization (post-8u191 bypass path)
