...
- `cors cross origin misconfiguration` (related Skill; use skill_list to locate it) when JSON endpoints become readable cross-origin
- `oauth oidc misconfiguration` (related Skill; use skill_list to locate it) when login, account linking, or callback binding relies on OAuth state
