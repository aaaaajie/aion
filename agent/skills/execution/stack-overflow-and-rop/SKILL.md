...
description: Stack overflow and ROP playbook. Use when exploiting buffer overflows
  to hijack control flow via return address overwrite, ROP chains, ret2libc, ret2csu,
  ret2dlresolve, or SROP on Linux userland binaries.
---

# Stack Overflow And Rop Skill
...
- the task mentions `stack`
- the task mentions `overflow`
- the task mentions `rop`
...
- `format-string-exploitation` (related Skill; use skill_list to locate it) — leak canary/libc/PIE base via format string before triggering overflow
...
- `heap-exploitation` (related Skill; use skill_list to locate it) — when the vulnerability is in heap rather than stack

### Advanced Reference

Load [references/ROP_ADVANCED_TECHNIQUES.md](references/ROP_ADVANCED_TECHNIQUES.md) when you need:
