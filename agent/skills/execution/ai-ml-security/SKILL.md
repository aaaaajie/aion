---
name: ai-ml-security
description: AI/ML security playbook. Use when assessing model supply chain attacks
  (pickle RCE, poisoned weights), adversarial examples, model poisoning, model stealing,
  data privacy attacks (membership inference, model inversion), and autonomous agent
  security risks.
---
...
- `llm-prompt-injection` (related Skill; use skill_list to locate it) for LLM-specific prompt injection, jailbreaking, and tool abuse techniques
- `deserialization-insecure` (related Skill; use skill_list to locate it) for deeper coverage of Python pickle and general deserialization attack patterns
- `dependency-confusion` (related Skill; use skill_list to locate it) when the ML pipeline has supply chain risks via pip/npm package confusion
...
For detailed prompt injection techniques, see `llm-prompt-injection` (related Skill; use skill_list to locate it).
...
Covered in `llm-prompt-injection JAILBREAK_PATTERNS.md` (related Skill; use skill_list to locate it) Section 5.
...
│   ├── Yes → Load `llm-prompt-injection` (related Skill; use skill_list to locate it)
