...
description: HTTP request smuggling and desynchronization testing for CL.TE, TE.CL, TE.TE, HTTP/2 to HTTP/1 translation, front proxy/CDN behavior, client-side desync, cache poisoning, and message-framing differentials. Use for request smuggling, desync, Content-Length, Transfer-Encoding, H2.CL, H2.TE, proxy, and CDN cases.
---
...
- the task mentions `smuggling`
- the task mentions `http`
- the task mentions `desynchronization`
...
- `ghost-bits-cast-attack` (related Skill; use skill_list to locate it) when the HTTP client library is **Apache HttpClient <= 4.5.9** (HTTPCLIENT-1974/1978) — injecting `瘍瘊` (U+760D U+760A, low bytes `\r\n`) into a header value causes the underlying char-to-byte writer to emit a literal CRLF, splitting the request at the origin without relying on CL/TE disagreement
...
Also load [references/H2_SMUGGLING_VARIANTS.md](references/H2_SMUGGLING_VARIANTS.md) when you need:
...
- **Cache and path-key confusion** -> `Web Cache Deception` (related Skill; use skill_list to locate it).
