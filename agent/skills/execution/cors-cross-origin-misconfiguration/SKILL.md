...
description: CORS cross-origin security testing for reflected origins, credentialed browser reads, preflight policy bugs, origin allowlist bypasses, and authenticated API access. Use for CORS, cross-origin, Origin, ACAO, ACAC, credentials, preflight, browser, browser-based, browser credential, and SOP cases.
---

# Cors Cross Origin Misconfiguration Skill
...
- the task mentions `cors`
- the task mentions `cross`
- the task mentions `origin`
...
> **AI LOAD INSTRUCTION**: Use this skill when browsers can access authenticated APIs cross-origin. Focus on reflected origins, credentialed requests, wildcard trust, parser mistakes, and origin allowlist bypasses. For JSONP hijacking deep dives, same-origin policy internals, honeypot de-anonymization, and CORS vs JSONP comparison, load the companion [references/SCENARIOS.md](references/SCENARIOS.md).

### Extended Scenarios

Also load [references/SCENARIOS.md](references/SCENARIOS.md) when you need:
...
- Session or JSON action abuse: `csrf cross site request forgery` (related Skill; use skill_list to locate it)
- OAuth token leakage and callback binding: `oauth oidc misconfiguration` (related Skill; use skill_list to locate it)
- API auth context: `api auth and jwt abuse` (related Skill; use skill_list to locate it)
