...
- `deserialization-insecure` (related Skill; use skill_list to locate it) — for
  Apache Commons BCEL ClassLoader and Fastjson `\u`/`\x` escape variants.
- `path-traversal-lfi` (related Skill; use skill_list to locate it) — Spring, Jetty,
  Undertow, Vert.x URL decoding and `%2>` hex folding.
- `upload-insecure-files` (related Skill; use skill_list to locate it) — Tomcat
  `RFC2231Utility` `filename*` Webshell upload.
- `request-smuggling` (related Skill; use skill_list to locate it) — Apache HttpClient
...
- `sqli-sql-injection` (related Skill; use skill_list to locate it) — Jackson `charToHex`
  table-lookup truncation hides SQL keywords inside Unicode escapes.

### Advanced Reference

Load `references/PAYLOAD_COOKBOOK.md` (resource: `references/PAYLOAD_COOKBOOK.md`) when you need:
...
Pair with `sqli-sql-injection` (related Skill; use skill_list to locate it) for the
...
Cross-reference `request-smuggling` (related Skill; use skill_list to locate it) for
...
See `references/PAYLOAD_COOKBOOK.md` (resource: `references/PAYLOAD_COOKBOOK.md`) for affected-version
