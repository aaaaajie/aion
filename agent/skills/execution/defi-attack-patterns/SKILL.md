...
- the task mentions `defi`
- the task mentions `attack`
- the task mentions `patterns`
- the task mentions `pattern`
...
- `smart-contract-vulnerabilities` (related Skill; use `skill_list` to locate it) for underlying Solidity vulnerability patterns (reentrancy, integer overflow, delegatecall)
- `deserialization-insecure` (related Skill; use `skill_list` to locate it) when targeting off-chain bridge relayer or indexer infrastructure
...
│   └── No → Continue
└── Load `smart-contract-vulnerabilities` (related Skill; use `skill_list` to locate it)
