...
- Apply all 13 checklist items from `false-positive-patterns.md` (related Skill; use `skill_list` to locate it) to each bug
...
**1.1**: Map trust boundaries (internal/trusted vs external/untrusted) and trace data from source to alleged vulnerability. Apply class-specific verification from `bug-class-verification.md` (related Skill; use `skill_list` to locate it). **Key pitfall**: Analyzing code in isolation without tracing the full validation chain. Conditional logic upstream may make the vulnerable code mathematically unreachable (see `false-positive-patterns.md` (related Skill; use `skill_list` to locate it) items 1 and 1a).
...
**2.2**: Create explicit algebraic proofs for bounds-related issues. Use the template in `evidence-templates.md` (related Skill; use `skill_list` to locate it). Verify: IF validation_check_passes THEN bounds_guarantee_holds.
...
See `evidence-templates.md` (related Skill; use `skill_list` to locate it) for PoC templates.
...
See `evidence-templates.md` (related Skill; use `skill_list` to locate it) for the devil's advocate documentation template.

## Gate Review

Apply the six gates from `gate-reviews.md` (related Skill; use `skill_list` to locate it) to reach a verdict.
