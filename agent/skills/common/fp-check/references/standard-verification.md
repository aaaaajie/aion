...
Two points in this checklist may trigger escalation to `deep-verification.md` (related Skill; use `skill_list` to locate it):
...
- Apply class-specific checks from `bug-class-verification.md` (related Skill; use `skill_list` to locate it)
...
- **Bounds proof**: For integer/bounds issues, create an explicit algebraic proof using the template in `evidence-templates.md` (related Skill; use `skill_list` to locate it). Verify: IF validation_check_passes THEN bounds_guarantee_holds.
...
See `evidence-templates.md` (related Skill; use `skill_list` to locate it) for the full PoC template.
...
Apply all six gates from `gate-reviews.md` (related Skill; use `skill_list` to locate it) and all 13 items from `false-positive-patterns.md` (related Skill; use `skill_list` to locate it) to reach a verdict.
