...
from scripts.runtime_web.tasks import task_summaries, task_detail
import logging
import sqlite3
import threading
import time
...
def _agent(row: sqlite3.Row, *, include_runtime: bool = False) -> dict[str, Any]:
    result: dict[str, Any] = {
        ...
        "last_model_activity_at": _iso(_row_value(row, "last_model_activity_at")),
        "last_tool_activity_at": _iso(_row_value(row, "last_tool_activity_at")),
        "waiting_sources": [],
...
def _waiting_sources(connection: sqlite3.Connection, run_id: str, agent_id: str) -> list[dict[str, Any]]:
    """Project only durable producers that can wake the Agent."""
    rows: list[dict[str, Any]] = []
    rows.extend(
        {"kind": row[2], "id": row[0], "status": row[1]}
        for row in connection.execute(
            "SELECT agent_id,status,role FROM agents WHERE run_id=? AND parent_id=? "
            "AND role IN ('worker','solver') AND status IN ('queued','pending','running','starting','working','waiting') "
            "ORDER BY created_at, agent_id",
            (run_id, agent_id),
        )
    )
    rows.extend(
        {"kind": "shell", "id": row[0], "status": row[1]}
        for row in connection.execute(
            "SELECT task_id,status FROM shell_tasks WHERE run_id=? AND agent_id=? AND status='running' "
            "ORDER BY created_at, task_id",
            (run_id, agent_id),
        )
    )
    rows.extend(
        {"kind": "network", "id": row[0], "status": row[1]}
        for row in connection.execute(
            "SELECT task_id,status FROM network_tasks WHERE run_id=? AND agent_id=? "
            "AND status IN ('queued','running','waiting') ORDER BY created_at, task_id",
            (run_id, agent_id),
        )
    )
    rows.extend(
        {"kind": "http", "id": row[0], "status": row[1], "analysis_status": row[2]}
        for row in connection.execute(
            "SELECT interaction_id,execution_status,analysis_status FROM http_interactions "
            "WHERE run_id=? AND agent_id=? AND "
            "(execution_status IN ('queued','running','waiting') OR analysis_status IN ('queued','running')) "
...
class _ReadOnlyStore:
    def __init__(self, database_path: Path, run_id: str, workspace_root: Path | None = None) -> None:
    ...
    def snapshot(self, after_sequence: int = 0) -> dict[str, Any]:
        ...
        try:
            ...
            for row in connection.execute(
                ...
                item = _agent(row)
                item["waiting_sources"] = _waiting_sources(
                    connection, self.run_id, row["agent_id"]
                )
                agents.append(item)
            findings = [
                _finding(row)
            ...
            usage_events = [
                dict(row)
                for row in connection.execute(
                    "SELECT agent_id,event_type,payload FROM state_events WHERE run_id=? AND event_type IN ('model_call_started','model_call_finished') ORDER BY sequence",
                    (self.run_id,),
                )
            ]
            token_usage = aggregate_usage(usage_events, agents)
    ...
    def task_detail(self, kind, task_id, *, offset=0, limit=10000):
        connection = self._connect()
        try:
            connection.execute("BEGIN")
            return _redact(task_detail(connection, self.run_id, kind, task_id,
                                      self.workspace_root, offset=offset, limit=limit))
        finally:
            connection.rollback()
            connection.close()
    ...
    def agent_detail(
        ...
        try:
            ...
            return {
                "agent": {
                    **_agent(row, include_runtime=True),
                    "waiting_sources": _waiting_sources(
                        connection, self.run_id, row["agent_id"]
                    ),
                },
                "events": events,
...
class RuntimeMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self.host = host
        self.port = port
        self._store = _ReadOnlyStore(self.database_path, run_id, workspace_root)
    ...
    def _snapshot_for(self, after_sequence: int) -> dict[str, Any]:
        ...
        earliest = min(
            (item["sequence"] for item in self._snapshot.get("events", [])), default=0
        )  # type: ignore[union-attr]
        cached["events_truncated"] = bool(earliest and earliest > after_sequence + 1)
        cached["events_gap_after"] = (
            after_sequence if cached["events_truncated"] else None
        )
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _dispatch(self, *, send_body: bool) -> None:
                ...
                if ".." in Path(path).parts:
                    self._send_json(
                        {"error": "invalid_path"}, HTTPStatus.BAD_REQUEST, send_body
                    )
                    return
                ...
                try:
                    if path == "/" or path == "/index.html":
                        ...
                        self._send_file(
                            "app.js", "text/javascript; charset=utf-8", send_body
                        )
                    elif path == "/assets/styles.css":
                        self._send_file(
                            "styles.css", "text/css; charset=utf-8", send_body
                        )
                    elif path in {
                        ...
                        self._send_json(
                            monitor._snapshot_for(after), HTTPStatus.OK, send_body
                        )
                    elif path.startswith("/api/tasks/"):
                        parts = path.removeprefix("/api/tasks/").split("/")
                        if len(parts) != 2:
                            raise LookupError("task_not_found")
                        detail = monitor._store.task_detail(*parts,
                            offset=self._int_param(params, "offset", 0),
                            limit=self._int_param(params, "limit", 10000))
                        ...
                        self._send_json(
                            {"error": "not_found"}, HTTPStatus.NOT_FOUND, send_body
                        )
                except LookupError:
                    self._send_json(
                        {"error": "agent_not_found"}, HTTPStatus.NOT_FOUND, send_body
                    )
                except (ValueError, TypeError):
                    self._send_json(
                        {"error": "invalid_query"}, HTTPStatus.BAD_REQUEST, send_body
                    )
            ...
            @staticmethod
            def _int_param(
                params: dict[str, list[str]], name: str, default: int
            ) -> int:
            ...
            def _send_file(
                self, filename: str, content_type: str, send_body: bool
            ) -> None:
            ...
            def _send_json(
                self, payload: Any, status: HTTPStatus, send_body: bool
            ) -> None:
                content = json.dumps(
