...
func TestScanEachWithControllerStreamsAndCompletes(t *testing.T) {
	listener := startTestFTPListener(t)
	defer listener.Close()
	...
	var streamed int
	ctrl, statsCh, errCh := scanner.ScanEachWithController(
		ctx,
		func(Result) error {
			streamed++
			return nil
	...
	if streamed == 0 {
		t.Fatal("expected streamed results")
	}
	if stats.TasksCompleted == 0 {
		t.Fatalf("expected completed tasks: %+v", stats)
	}
}
