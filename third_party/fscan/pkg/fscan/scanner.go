...
// ScanEachWithController streams results without retaining them and returns a
// controller for cooperative pause/resume. The stats and error channels each
// receive exactly one value when the scan ends.
func (s *Scanner) ScanEachWithController(ctx context.Context, handle ResultHandler, targets ...Target) (*ScanController, <-chan ScanStats, <-chan error) {
	ctrl := newScanController()
	statsCh := make(chan ScanStats, 1)
	errCh := make(chan error, 1)

	go func() {
		stats, err := s.scanEach(ctx, scanOpts{controller: ctrl}, handle, targets...)
		statsCh <- stats
		errCh <- err
		close(statsCh)
		close(errCh)
...
func (s *Scanner) scanOne(ctx context.Context, target Target, sink common.ResultSink, opts scanOpts) (core.ScanReport, error) {
	...
	cfg.DisablePlugins = s.config.DisablePlugins
...
func buildFlagVars(config Config, target Target) *common.FlagVars {
	...
	return &common.FlagVars{
		...
		TimeoutSec:      timeout,
		// Embedded callers own cancellation and task duration through ctx.
		GlobalTimeout:       0,
