...
func TestBaseScanStrategyPluginSelectionAndApplicability(t *testing.T) {
	...
	disabled, custom := strategy.GetPlugins(cfg)
	if !custom || len(disabled) != 0 {
		t.Fatalf("disabled plugins = %#v custom=%v, want empty custom selection", disabled, custom)
	}
	cfg.DisablePlugins = false
